...
if len(sys.argv)!=3:
    print("Please enter: run_motion_alignment.py log_dir frame_no")
    exit()
else:
    log_dir = sys.argv[1]
...
#result log path
result_dir = log_dir + '/trajectory_alignment_frame_%02d'%frame_no
...
num_of_frame = min(len(mv_from_image),len(vid_meta))-1
...
plt.title('alignment %d frame, trjactory between image_x and gyro yaw'%(min_traj_idx+1))
plt.legend()
...
plt.title('alignment %d frame, trjactory between image_y and gyro pitch'%(min_traj_idx+1))
plt.legend()
...
plt.title('alignment %d frame, trjactory between image_z and gyro roll'%(min_traj_idx+1))
plt.legend()
